...
void AP_MotorsMatrix::init(motor_frame_class frame_class,
  ...
  if (frame_class == MOTOR_FRAME_SCRIPTING_MATRIX) {
    // if Scripting frame class, do nothing scripting must call its own
...
bool AP_MotorsMatrix::set_throttle_factor(int8_t motor_num,
...
void AP_MotorsMatrix::set_frame_class_and_type(motor_frame_class frame_class,
  ...
  if (armed() || (frame_class == _active_frame_class &&
                  _active_frame_type == frame_type)) {
    return;
  }
  _active_frame_class = frame_class;
...
// (1 means being used)
//  this can be used to ensure other pwm outputs (i.e. for servos) do not
//  conflict
...
// helper to return value scaled between boost and normal based on the value of
// _thrust_boost_ratio _thrust_boost_ratio of 1 -> return = boost_value
// _thrust_boost_ratio of 0 -> return = normal_value
float AP_MotorsMatrix::boost_ratio(float boost_value,
                                   float normal_value) const {
  return _thrust_boost_ratio * boost_value +
         (1.0 - _thrust_boost_ratio) * normal_value;
}
...
void AP_MotorsMatrix::output_armed_stabilizing() {
  ...
  const float compensation_gain =
      thr_lin.get_compensation_gain(); // compensation for battery voltage and
                                       // altitude
  ...
  // throttle thrust maximum value, 0.0 - 1.0, If thrust boost is active then do
  // not limit maximum thrust
  const float throttle_thrust_max =
      boost_ratio(1.0, _throttle_thrust_max * compensation_gain);
  ...
  // ensure that throttle_avg_max is between the input throttle and the maximum
  // throttle
  throttle_avg_max =
      constrain_float(throttle_avg_max, throttle_thrust, throttle_thrust_max);

  // throttle providing maximum roll, pitch and yaw range
  // calculate the highest allowed average thrust that will provide maximum
  // control range
  float throttle_thrust_best_rpy = MIN(0.5f, throttle_avg_max);

  // calculate throttle that gives most possible room for yaw which is the lower
  // of:
  //      1. 0.5f - (rpy_low+rpy_high)/2.0 - this would give the maximum
  //      possible margin above the highest motor and below the lowest
  //      2. the higher of:
  //            a) the pilot's throttle input
  //            b) the point _throttle_rpy_mix between the pilot's input
  //            throttle and hover-throttle
  //      Situation #2 ensure we never increase the throttle above hover
  //      throttle unless the pilot has commanded this. Situation #2b allows us
  //      to raise the throttle above what the pilot commanded but not so far
  //      that it would actually cause the copter to rise. We will choose #1
  //      (the best throttle for yaw control) if that means reducing throttle to
  //      the motors (i.e. we favor reducing throttle *because* it provides
  //      better yaw control) We will choose #2 (a mix of pilot and hover
  //      throttle) only when the throttle is quite low.  We favor reducing
  //      throttle instead of better yaw control because the pilot has commanded
  //      it

  // Under the motor lost condition we remove the highest motor output from our
  // calculations and let that motor go greater than 1.0 To ensure control and
  // maximum righting performance Hex and Octo have some optimal settings that
  // should be used Y6               : MOT_YAW_HEADROOM = 350, ATC_RAT_RLL_IMAX
  // = 1.0,   ATC_RAT_PIT_IMAX = 1.0,   ATC_RAT_YAW_IMAX = 0.5 Octo-Quad (x8) x
  // : MOT_YAW_HEADROOM = 300, ATC_RAT_RLL_IMAX = 0.375, ATC_RAT_PIT_IMAX =
  // 0.375, ATC_RAT_YAW_IMAX = 0.375 Octo-Quad (x8) + : MOT_YAW_HEADROOM = 300,
  // ATC_RAT_RLL_IMAX = 0.75,  ATC_RAT_PIT_IMAX = 0.75,  ATC_RAT_YAW_IMAX =
  // 0.375 Usable minimums below may result in attitude offsets when motors are
  // lost. Hex aircraft are only marginal and must be handles with care Hex :
  // MOT_YAW_HEADROOM = 0,   ATC_RAT_RLL_IMAX = 1.0,   ATC_RAT_PIT_IMAX = 1.0,
  // ATC_RAT_YAW_IMAX = 0.5 Octo-Quad (x8) x : MOT_YAW_HEADROOM = 300,
  // ATC_RAT_RLL_IMAX = 0.25,  ATC_RAT_PIT_IMAX = 0.25,  ATC_RAT_YAW_IMAX = 0.25
  // Octo-Quad (x8) + : MOT_YAW_HEADROOM = 300, ATC_RAT_RLL_IMAX = 0.5,
  // ATC_RAT_PIT_IMAX = 0.5,   ATC_RAT_YAW_IMAX = 0.25 Quads cannot make use of
  // motor loss handling because it doesn't have enough degrees of freedom.

  // calculate amount of yaw we can fit into the throttle range
  // this is always equal to or less than the requested yaw from the pilot or
  // rate controller
  ...
  for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
    if (motor_enabled[i]) {
      ...
      _thrust_rpyt_out[i] =
          roll_thrust * _roll_factor[i] + pitch_thrust * _pitch_factor[i];
      ...
      if (!is_zero(_yaw_factor[i]) &&
          (!_thrust_boost || i != _motor_lost_index)) {
  ...
  // Include the lost motor scaled by _thrust_boost_ratio to smoothly transition
  // this motor in and out of the calculation
  if (_thrust_boost && motor_enabled[_motor_lost_index]) {
    // Check the maximum yaw control that can be used on this channel
    // Exclude any lost motors if thrust boost is enabled
    if (!is_zero(_yaw_factor[_motor_lost_index])) {
      const float thrust_rp_best_throttle =
          throttle_thrust_best_rpy + _thrust_rpyt_out[_motor_lost_index];
      float motor_room;
      ...
      const float motor_yaw_allowed =
          MAX(motor_room, 0.0) / fabsf(_yaw_factor[_motor_lost_index]);
      yaw_allowed =
          boost_ratio(yaw_allowed, MIN(yaw_allowed, motor_yaw_allowed));
    }
  }
  ...
  if (_thrust_boost) {
    // record highest roll + pitch + yaw command
    if (_thrust_rpyt_out[_motor_lost_index] > rpy_high &&
        motor_enabled[_motor_lost_index]) {
      rpy_high = boost_ratio(rpy_high, _thrust_rpyt_out[_motor_lost_index]);
    }
  }

  // calculate any scaling needed to make the combined thrust outputs fit within
  // the output range
  ...
  if (rpy_scale < 1.0f) {
    ...
    // todo: add lower limit flag and ensure it is handled correctly in altitude
    // controller
    thr_adj = 0.0f;
...
//      a) throttle_thrust_best_rpy : throttle level (from 0 to 1) providing
//      maximum roll, pitch and yaw range without climbing b) thr_adj: the
//      difference between the pilot's desired throttle and
//      throttle_thrust_best_rpy
//   records filtered motor output values in _thrust_rpyt_out_filt array
//   sets thrust_balanced to true if motors are balanced, false if a motor
//   failure is detected sets _motor_lost_index to index of failed motor
void AP_MotorsMatrix::check_for_failed_motor(
  ...
  for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
    if (motor_enabled[i]) {
      _thrust_rpyt_out_filt[i] +=
          alpha * (_thrust_rpyt_out[i] - _thrust_rpyt_out_filt[i]);
    }
  }
  ...
  // ensure thrust balance does not activate for multirotors with less than 6
  // motors
  ...
  // check to see if thrust boost is using more throttle than
  // _throttle_thrust_max
  if ((_throttle_thrust_max * thr_lin.get_compensation_gain() >
       throttle_thrust_best_plus_adj) &&
      (rpyt_high < 0.9f) && _thrust_balanced) {
    _thrust_boost = false;
  }
}

// output_test_seq - spin a motor at the pwm value specified
//  motor_seq is the motor's sequence number from 1 to the number of motors on
//  the frame pwm value is an actual pwm value that will be output, normally in
//  the range of 1000 ~ 2000
void AP_MotorsMatrix::_output_test_seq(uint8_t motor_seq, int16_t pwm) {
  // loop through all the possible orders spinning any motors that match that
  // description
  for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
...
//  pwm value is an actual pwm value that will be output, normally in the range
//  of 1000 ~ 2000
...
// add_motor
void AP_MotorsMatrix::add_motor_raw(int8_t motor_num, float roll_fac,
                                    float pitch_fac, float yaw_fac,
                                    uint8_t testing_order,
                                    float throttle_factor) {
  if (initialised_ok()) {
    // do not allow motors to be set if the current frame type has init
    // correctly
    return;
  }
...
// add_motor using just position and prop direction - assumes that for each
// motor, roll and pitch factors are equal
void AP_MotorsMatrix::add_motor(int8_t motor_num, float angle_degrees,
                                float yaw_factor, uint8_t testing_order) {
  add_motor(motor_num, angle_degrees, angle_degrees, yaw_factor, testing_order);
}

// add_motor using position and prop direction. Roll and Pitch factors can
// differ (for asymmetrical frames)
void AP_MotorsMatrix::add_motor(int8_t motor_num, float roll_factor_in_degrees,
                                float pitch_factor_in_degrees, float yaw_factor,
                                uint8_t testing_order) {
  add_motor_raw(motor_num, cosf(radians(roll_factor_in_degrees + 90)),
                cosf(radians(pitch_factor_in_degrees)), yaw_factor,
                testing_order);
}

// remove_motor - disabled motor and clears all roll, pitch, throttle factors
// for this motor
...
void AP_MotorsMatrix::add_motors(const struct MotorDef *motors,
                                 uint8_t num_motors) {
...
void AP_MotorsMatrix::add_motors_raw(const struct MotorDefRaw *motors,
                                     uint8_t num_motors) {
  for (uint8_t i = 0; i < num_motors; i++) {
...
bool AP_MotorsMatrix::setup_quad_matrix(motor_frame_type frame_type) {
  ...
  case MOTOR_FRAME_TYPE_NYT_X: {
    ...
    break;
       ...
       // APM_BUILD_TYPE(APM_BUILD_UNKNOWN)
  ...
  case MOTOR_FRAME_TYPE_DJI_X: {
    // DJI quad X order
    // see
    // https://forum44.djicdn.com/data/attachment/forum/201711/26/172348bppvtt1ot1nrtp5j.jpg
  ...
  case MOTOR_FRAME_TYPE_VTAIL: {
    /*
        ...
        Roll control comes only from the front motors, Yaw control only from the
       rear motors. Roll & Pitch factor is measured by the angle away from the
       top of the forward axis to each arm.

        Note: if we want the front motors to help with yaw,
            motors 1's yaw factor should be changed to sin(radians(40)).  Where
       "40" is the vtail angle motors 3's yaw factor should be changed to
       -sin(radians(40))
    */
  ...
  case MOTOR_FRAME_TYPE_ATAIL:
    /*
        The A-Shaped VTail is the exact same as a V-Shaped VTail, with one
       difference:
        - The Yaw factors are reversed, because the rear motors are facing
...
bool AP_MotorsMatrix::setup_octaquad_matrix(motor_frame_type frame_type) {
  ...
  case MOTOR_FRAME_TYPE_V: {
    _frame_type_string = "V";
    static const AP_MotorsMatrix::MotorDef motors[]{
        {45, 0.7981f, 1},   {-45, -0.7981f, 7},  {-135, 1.0000f, 5},
        {135, -1.0000f, 3}, {-45, 0.7981f, 8},   {45, -0.7981f, 2},
        {135, 1.0000f, 4},  {-135, -1.0000f, 6},
    };
    add_motors(motors, ARRAY_SIZE(motors));
    break;
  }
...
bool AP_MotorsMatrix::setup_y6_matrix(motor_frame_type frame_type) {
  ...
  case MOTOR_FRAME_TYPE_Y6B: {
    // Y6 motor definition with all top motors spinning clockwise, all bottom
...
bool AP_MotorsMatrix::get_factors(uint8_t i, float &roll, float &pitch,
                                  float &yaw, float &throttle,
                                  uint8_t &testing_order) const {
